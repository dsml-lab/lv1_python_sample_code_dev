...
from src.evaluation_lv2 import IMAGE_SIZE
from src.evaluation_lv2 import LV2_Evaluator

from src.labels_lv2 import ID2LNAME
from src.labels_lv2 import N_LABELS

AREA_COLOR = (128,128,255)
...
def LV1_user_accuracy_plot(accuracy_list, n, path):
    label = [i / 255 for i in AREA_COLOR]
...
def lv2_user_load_directory(path):
...
def main():
    ...
    for max_value in range(100, 1000, 300):
        for method_name in method_names:
            ...
            path = './lv2_targets'
            target_image = lv2_user_load_directory(path)
            # print(target_image)
            ...
            for i in target_image:
                ...
                for n in N:
                    ...
                    features, labels = lv2_user_function_sampling_democracy(n_samples=n, target_model=target,
                                                                        exe_n=n)
                    # else:
                    #     raise ValueError
                    ...
                    # accuracyを配列に格納。
                    r_avg, p_avg, f_avg = evaluator.calc_accuracy(target, model)
                    accuracy_list.append(f_avg)

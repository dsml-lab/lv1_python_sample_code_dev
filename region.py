...
def lv1_user_function_sampling_sweeper_colorless(n_samples, target_model, exe_n, model_name: str):
    ...
    if n_samples < 0:
        ...
        # clone識別器からcloneのラベルを取得
        clone = LV1UserDefinedClassifier(n=n_samples, model_name=model_name)
        # 学習
        ...
        for old_feature, target_label, clone_label in zip(old_features, target_labels, clone_labels):
            ...
            if target_label == clone_label:
                new_board.open_once_feature(feature_x=old_feature[0], feature_y=old_feature[1], color=target_label)  # 開示
            else:
